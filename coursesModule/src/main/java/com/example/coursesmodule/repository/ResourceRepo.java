...
public interface ResourceRepo extends JpaRepository<Resource, Integer> {
    ...
    @Query("SELECT r FROM Subject s JOIN s.componentList c JOIN c.resources r WHERE s.title = ?1 AND s.isDeleted = FALSE AND c.isDeleted = FALSE AND r.isDeleted = FALSE")
    List<Resource> findAllBySubjectTitle(String title);
