...
import com.example.dao.SubjectDataAccessService;
import com.example.models.Component;
import com.example.models.Evaluation;
import com.example.models.Resource;
import com.example.models.Subject;
import com.example.repository.ComponentRepo;
import com.example.repository.EvaluationRepo;
...
class SubjectDataAccessServiceTest {
    ...
    @Test
    void insertSubject(){
        Subject testSubject = new Subject(1, "Test Subject", 6, 1, 2, "Test Description",
    ...
    void deleteSubjectByTitleOldImageNotNull(){
        Subject subject = new Subject(1, "Test Subject", 6, 1, 2, "Test Description",
                new ArrayList<>(), new ArrayList<>(), false);
    ...
    void updateSubjectByTitleWhereNewTitleIsSame(){
        // given
        Subject subject = new Subject(1, "Math", 4, 1, 1, "Math subject",
                new ArrayList<>(), new ArrayList<>(), false);
        Component component = new Component(1,"Course", 10, List.of(new Resource("Lecture 1", "/resources/Math/lectures/Lecture1.txt", "txt", false)), false);
        subject.setComponentList(List.of(component));
        Mockito.when(subjectRepo.findSubjectByTitle("Math")).thenReturn(Optional.of(subject));

        // when
        Subject newSubject = new Subject(1,"Math", 5, 1, 2, "Mathematics subject",
                new ArrayList<>(), new ArrayList<>(), false);
        int result = subjectDas.updateSubjectByTitle("Math", newSubject);
    ...
    public void testUpdateSubjectWithOldImage() {
        ...
        Subject subject = new Subject(1, "Title", 6, 1, 2, "Description",
                new ArrayList<>(), new ArrayList<>(), false);
        subject.setImage(image);
        subjectRepo.save(subject);
    ...
    void saveImageToSubject_WithOldImage() {
        Subject subject = new Subject(1, "Math", 3, 2023, 2, "Mathematics subject", null, null, false);
        ...
        Resource updatedImage = subject.getImage();
        assertNotNull(updatedImage);
        Resource savedImage = subjectRepo.findSubjectByTitle("Math").orElse(null).getImage();
    ...
    void deleteComponentByTypeSuccessful(){
        ...
        Subject subject = new Subject(1, "Test Subject", 6, 1, 2, "Test Description",
                new ArrayList<>(), new ArrayList<>(), false);
        Component component = new Component(1, type, 10, new ArrayList<>(), false);
    ...
    void addResourceForComponentTypeComponentNotFound(){
        Resource resource = new Resource(1, "Test Resource", "Test Link", "Test Description", false);
    ...
    void addEvaluationMethodSubjectNotFound(){
        Evaluation evaluation = new Evaluation(1L, "Course", 0.5F, "description", false);
    ...
    void addEvaluationSuccessful(){
        ...
        Evaluation evaluation = new Evaluation(1L, type, 0.5F, "description", false);
    ...
    void updateEvaluationMethodByComponentSuccessful(){
        ...
        Subject subject = new Subject(1, title, 6, 1, 2, "Test Description",
                new ArrayList<>(), new ArrayList<>(), false);
        subjectRepo.save(subject);
        subjectDas.addEvaluationMethod(title, evaluation);
        Mockito.when(evaluationRepo.findBySubjectTitleAndComponent(title, type)).thenReturn(Optional.of(evaluation));

        Evaluation updatedEvaluation = new Evaluation(1L, type, 0.5F, "updated description", false);
